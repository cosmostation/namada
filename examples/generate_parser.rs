...
use namada_sdk::borsh::schema::Definition;
use namada_sdk::borsh::schema::Fields;
use namada_sdk::borsh::schema::Declaration;
use namada_sdk::borsh::schema::FieldName;
...
fn is_fixed_sized_array(
    ...
    match defs.get(decl)? {
        Definition::Sequence { length_width, length_range, elements } if
            *length_width == 0 && length_range.start() == length_range.end() =>
...
fn mangle_name(decl: &Declaration) -> Declaration {
    decl.replace(&['<', '>', '(', ')', '[', ']'], "").replace(", ", "_").replace("; ", "_").replace("::", "_")
}
...
fn process_field(
    ...
    if let Some(_decl) = is_unit(&decl, &definitions) {
        ...
        parse_instrs.push(format!("  CHECK_ERROR({}(ctx, &obj->{}))", parser, mangle_name(name)));
        ...
        if let Some((decl, parser)) = is_primitive(&decl) {
            if decl == "uint8_t" {
                parse_instrs.push(format!("  CHECK_ERROR(readBytesAlt(ctx, obj->{}, {}))", mangle_name(name), len));
            } else {
                parse_instrs.push(format!("  for(uint32_t i = 0; i < {}; i++) {{", len));
                parse_instrs.push(format!("    {}(ctx, &obj->{}[i])", parser, mangle_name(name)));
                parse_instrs.push(format!("  }}"));
            }
            struct_decls.push(format!("  {} {}[{}];", decl, mangle_name(name), len));
        } else if let Some(decl) = is_tuple(&decl, &definitions) {
            struct_decls.push(format!("  {} {}[{}];", mangle_name(&decl), mangle_name(name), len));
        } else {
            struct_decls.push(format!("  {} {}[{}];", mangle_name(&decl), mangle_name(name), len));
        }
    } else if let Some((decl, length_tag_len)) = is_variable_length_array(decl, &definitions) {
        let length_tag = match length_tag_len {
            0 => {
                format!("unk_size")
            },
            1 => {
                struct_decls.push(format!("  uint8_t {}Len;", mangle_name(name)));
                parse_instrs.push(format!("  CHECK_ERROR(readByte(ctx, &obj->{}Len))", mangle_name(name)));
                format!("obj->{}Len", mangle_name(name))
            },
            2 => {
                struct_decls.push(format!("  uint16_t {}Len;", mangle_name(name)));
                parse_instrs.push(format!("  CHECK_ERROR(readUint16(ctx, &obj->{}Len))", mangle_name(name)));
                format!("obj->{}Len", mangle_name(name))
            },
            4 => {
                struct_decls.push(format!("  uint32_t {}Len;", mangle_name(name)));
                parse_instrs.push(format!("  CHECK_ERROR(readUint32(ctx, &obj->{}Len))", mangle_name(name)));
                format!("obj->{}Len", mangle_name(name))
            },
            8 => {
                struct_decls.push(format!("  uint64_t {}Len;", mangle_name(name)));
                parse_instrs.push(format!("  CHECK_ERROR(readUint64(ctx, &obj->{}Len))", mangle_name(name)));
                format!("obj->{}Len", mangle_name(name))
            },
            _ => panic!("invalid length tag length")
        };
        if let Some((decl, parser)) = is_primitive(&decl) {
            parse_instrs.push(format!("  if((obj->{} = mem_alloc({} * sizeof({}))) == NULL) {{", mangle_name(name), length_tag, decl));
            parse_instrs.push(format!("    return parser_unexpected_error;"));
            parse_instrs.push(format!("  }}"));
            parse_instrs.push(format!("  for(uint32_t i = 0; i < {}; i++) {{", length_tag));
            parse_instrs.push(format!("    CHECK_ERROR({}(ctx, &obj->{}[i]))", parser, mangle_name(name)));
            parse_instrs.push(format!("  }}"));
            struct_decls.push(format!("  {} *{};", decl, mangle_name(name)));
        } else if let Some(decl) = is_tuple(&decl, &definitions) {
            parse_instrs.push(format!("  if((obj->{} = mem_alloc({} * sizeof({}))) == NULL) {{", mangle_name(name), length_tag, mangle_name(&decl)));
            parse_instrs.push(format!("    return parser_unexpected_error;"));
            parse_instrs.push(format!("  }}"));
            parse_instrs.push(format!("  for(uint32_t i = 0; i < {}; i++) {{", length_tag));
            parse_instrs.push(format!("    CHECK_ERROR(read{}(ctx, &obj->{}[i]))", mangle_name(&decl), mangle_name(name)));
            parse_instrs.push(format!("  }}"));
            struct_decls.push(format!("  {} *{};", mangle_name(&decl), mangle_name(name)));
        } else if let Some((decl, len)) = is_fixed_sized_array(&decl, &definitions) {
            if let Some((decl, parser)) = is_primitive(&decl) {
                if decl == "uint8_t" {
                    parse_instrs.push(format!("  if((obj->{} = mem_alloc({} * sizeof({}[{}]))) == NULL) {{", mangle_name(name), length_tag, decl, len));
                    parse_instrs.push(format!("    return parser_unexpected_error;"));
                    parse_instrs.push(format!("  }}"));
                    parse_instrs.push(format!("  for(uint32_t i = 0; i < {}; i++) {{", length_tag));
                    parse_instrs.push(format!("    CHECK_ERROR(readBytesAlt(ctx, obj->{}[i], {}))", mangle_name(name), len));
                    parse_instrs.push(format!("  }}"));
                }
                struct_decls.push(format!("  {} (*{})[{}];", decl, mangle_name(name), len));
            } else if let Some(decl) = is_tuple(&decl, &definitions) {
        ...
        parse_instrs.push(format!("  CHECK_ERROR(read{}(ctx, &obj->{}))", mangle_name(&decl), mangle_name(name)));
        struct_decls.push(format!("  {} {};", mangle_name(&decl), mangle_name(name)));
    }
}
    ...
    for (declaration, definition) in &definitions {
        ...
        match definition {
            Definition::Struct { fields: Fields::Empty } => {
                struct_decls.push(format!("typedef struct {{}} {};", declaration));
            ...
            Definition::Struct { fields: Fields::UnnamedFields(fields) } => {
                ...
                for (idx, decl) in fields.iter().enumerate() {
                    process_field(&format!("f{}", idx), decl, &definitions, &mut struct_decls, &mut parse_instrs);
                }
                struct_decls.push(format!("}} {};", declaration));
                parse_instrs.push(format!("  return parser_ok;"));
                parse_instrs.push(format!("}}"));
            },
            Definition::Struct { fields: Fields::NamedFields(fields) } => {
                struct_decls.push(format!("typedef struct {{"));
                parser_decls.push(format!(
                    "parser_error_t read{}(parser_context_t *ctx, {} *obj);",
                    declaration,
                ...
                for (name, decl) in fields {
                    process_field(name, decl, &definitions, &mut struct_decls, &mut parse_instrs);
                }
                struct_decls.push(format!("}} {};", declaration));
                parse_instrs.push(format!("  return parser_ok;"));
                parse_instrs.push(format!("}}"));
            ...
            Definition::Enum { tag_width: 0, variants } => {
                parser_decls.push(format!(
                    "parser_error_t read{}(parser_context_t *ctx, {} *obj);",
                    declaration,
                    declaration,
            ...
            Definition::Enum { tag_width: 1, variants } => {
                parser_decls.push(format!(
                    "parser_error_t read{}(parser_context_t *ctx, {} *obj);",
                    declaration,
                    declaration,
                ...
                parse_instrs.push(format!("  CHECK_ERROR(readByte(ctx, &obj->tag))"));
            ...
            Definition::Enum { tag_width: 2, variants } => {
                struct_decls.push(format!("typedef struct {{"));
            ...
            Definition::Enum { tag_width: 4, variants } => {
                parser_decls.push(format!(
                    "parser_error_t read{}(parser_context_t *ctx, {} *obj);",
                    declaration,
                    declaration,
                ...
                parse_instrs.push(format!("  CHECK_ERROR(readUint32(ctx, &obj->tag))"));
                parse_instrs.push(format!("  switch(obj->tag) {{"));
                struct_decls.push(format!("typedef struct {{"));
